...
contract Pool is IPool, PoolStorage {
    ...
    function depositSavings(
        ...
        // only need to rebalance if asset is a debt asset
        // if asset depositing is a yield asset, it likely
        // will not be use in any of the routers
        // for example, UST is not a debtable asset and will not be routed
    ...
    //     // we will take the lowest outcome of the ctd vs. ltv checks
    ...
    //     // assets that are collateral and savings can be used as collateral
    //     // this will burn the savings asset amount and mint collateral asset
    //     if (!useSavings) {
    ...
    //         IAvaToken(collateralAsset.wrapped).burnTo(msg.sender, amount, collateralAsset.collateralAssetWrapped, collateralAsset.overallExchangeRate);
    //     }

    //     // ICollateralToken(collateralAsset.collateralAssetWrapped).mint(msg.sender, amount, collateralAsset.collateralExchangeRate);
    //     uint256 amountBack = ICollateralToken(collateralAsset.collateralAssetWrapped).mint(msg.sender, amount, collateralAsset.collateralExchangeRate);
    ...
    //     updateIsBorrowing(msg.sender, true);
    ...
    function depositCollateralAndBorrow(
        ...
        PoolAsset storage collateralAsset = poolAssets[_collateralAsset];
        PoolAsset storage debtAsset = poolAssets[_debtAsset];
        UserData storage userData = usersData[msg.sender][_debtAsset];
        ...
        if (!useSavings) {
            ...
            // savings assets that can be used as collateral are not debtable assets
            // therefor there is no interest rate abuse ability if a user `burnTo`s too much
            // underlying/wrapped asset will be sent in same tx so there is no interoperable tx delays which allows for no revert mint next
            IAvaToken(collateralAsset.wrapped).burnTo(msg.sender, amount, collateralAsset.collateralAssetWrapped, collateralAsset.overallExchangeRate);
        }

        uint256 amountBack = amount;
        (amountBack, ) = ICollateralToken(collateralAsset.collateralAssetWrapped).mint(msg.sender, amount, collateralAsset.collateralExchangeRate, !useSavings);

        PoolLogic.updateUserState(
            userData,
            collateralAsset,
            debtAsset,
            msg.sender,
        ...
        // console.log("collateralValueInEth", collateralValueInEth);

        // we will take the lowest outcome of the ctd vs. ltv checks
        // ex: 80,000 = 100,000 / 1.25
        uint256 maxBorrowValueInEthCtd = collateralValueInEth.wadDiv(collateralAsset.ctd);
        // console.log("maxBorrowValueInEthCtd", maxBorrowValueInEthCtd);
        // console.log("collateralAsset.ctd", collateralAsset.ctd);
        // ex: 100,000 = 100,000 * 1.0
        uint256 maxBorrowValueInEthLtv = collateralValueInEth.wadMul(debtAsset.ltv);
        // console.log("maxBorrowValueInEthLtv", maxBorrowValueInEthLtv);
        // console.log("debtAsset.ltv", debtAsset.ltv);
        // ex: 80,000
        uint256 maxBorrowValueInEth = maxBorrowValueInEthCtd < maxBorrowValueInEthLtv ? maxBorrowValueInEthCtd : maxBorrowValueInEthLtv;
    ...
    function repayAndWithdraw(
        ...
        if (repayValueInEth >= totalDebtInEth) {
            ...
            collateralWithdrawAmount = balance;
        ...
        PoolLogic.updateUserState(
            ...
            0,
            repayAmount,
            repayValueInEth >= totalDebtInEth,
            false
        );


        // avoid any yield miscalculation from other protocols if using yield asset as collateral
        collateralWithdrawAmount = collateralWithdrawAmount > balance ? balance : collateralWithdrawAmount;

        // if requiring to send to savings on repay
