...
contract AvaToken is RewardsTokenBase, IAvaToken {
  ...
  */
   ...
   * Total routed balanceOf and accrued debt delta
   * Note: This doesn't include total debt, just the accrued
  ...
  function routerSuppliedScaledBalance() external view override returns (uint256) {
      return _routerWrapped.balanceOf(address(this));
  }
   ...
   * This is a manually tracked version of balanceOf due to bridging
   **/
  ...
  function burnTo(address account, uint256 amount, address _to, uint256 exchangeRate) external override onlyPool {
      ...
      // send the `underlying` wrapped asset in contract which is the wrapped asset of the represending underlying
